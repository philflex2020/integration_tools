...
type Cmd struct {
	Key, Help string
	Func      func(*bufio.Reader, *Cmd, string, string) bool
}

func (c CMDS) AddCmd(key, help string, f func(*bufio.Reader, *Cmd, string, string) bool) int {
	c.Cmds[key] = Cmd{Key: key, Help: help, Func: f}
	return 0
}

// // crude line editor with just \b activated .. to give way for better stuff later
// // done replace '\b'
// //func (c CMDS) RunCmd(conn net.Conn, msg string) bool {

// // string.replace
// // strking.EndsWith
// //strings.HasSuffix(s1, "GeeksforGeeks!")
// // 	fo = strings.Replace(fo, "\r\n", "", -1)

func RunCmd(conn *bufio.Reader, msg string, sm ...string) bool {
	b := false
	...
	msg2 := []byte(msg)
	if len(msg2) < 1 || msg2[0] == '#' {
		return b
	}
	//xyz\b\b\bhelp
	//this snippet allows backspaces to work in telnet
	im := 0
	for i := 0; i < len(msg); i++ {
		if msg[i] != '\b' {
			msg2[im] = msg[i]
	...
	f := strings.Fields(string(message))
	if len(f) > 0 {
		fun, ok := Cmds.Cmds[f[0]]
		if ok {
			if Cmds.fd != nil {
				Cmds.fd.WriteString(string(message) + "\n")
			}
			fmt.Printf(" runCmd >>>> message [%s] smap [%s]\n", message, smap)
			b = fun.Func(conn, &fun, string(message), smap)
			// } else {
			// 	str := string(message) + " not understood, try \"help\""
			// 	conn.(str)
		}
	}
	return b
...
// this is the collection of system Commands , just maintained locally.

var Cmds *CMDS

func QuitFunc(conn *bufio.Reader, b *Cmd, c, s string) bool {
	str := b.Key + ": closing connection"
	fmt.Println(str)
	return true
}

func HelpFunc(conn *bufio.Reader, b *Cmd, c, s string) bool {
...
func main() {
	...
	fmt.Printf("Simple Shell reader %T \n", reader)
	Cmds = SetUpCmds()
	RunCmd(reader, "help")
