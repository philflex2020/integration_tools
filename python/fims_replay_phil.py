...
def usage():
    print("Usage info:\n    python3 fims_replay.py fims_output_file [prefix]")

# Parses a file containing a dump of fims messages
def parse_fims_dump(file_src, prefix):
    try:
        f_r = open(file_src)
    except OSError:
    ...
    with f_r:
        ...
        for line in lines:
            ...
            #"Timestamp":"04-30-2021 05:47:36.99319"}
                ...
                if  len(line.split("Timestamp:", 1)) < 2:
                    continue
                ...
                if methd and uri:
                    if methd == "pub" or methd == "set" or methd == "get":
                        ...
                        if text.lower() == "y" or text.lower() == "yes":
                            output_fims_msg(methd, uri, reply_to, body, prefix)
                methd, uri, reply_to, body = "", "", "", ""
...
def send_fims_msg(methd, uri, reply_to, body):
    if not body and not reply_to:
        subprocess.call(["/usr/local/bin/fims_send", "-m", methd, "-u", uri])
    elif body and not reply_to:
        subprocess.call(["/usr/local/bin/fims_send", "-m", methd, "-u", uri, body])
    elif not body and reply_to:
        subprocess.call(["/usr/local/bin/fims_send", "-m", methd, "-u", uri, "-r", reply_to])
    else:
        subprocess.call(["/usr/local/bin/fims_send", "-m", methd, "-u", uri, "-r", reply_to, body])

def output_fims_msg(methd, uri, reply_to, body, prefix):
    uri = prefix+uri
    if not body and not reply_to:
        print("/usr/local/bin/fims_send -m " + methd +" -u "+uri)
    elif body and not reply_to:
        print("/usr/local/bin/fims_send -m " + methd +" -u "+uri+" '"+body+"'")
    elif not body and reply_to:
        print("/usr/local/bin/fims_send -m " + methd +" -u "+uri+" -r "+reply_to)
    else:
        print("/usr/local/bin/fims_send -m " + methd +" -u "+uri+" -r "+reply_to+" '"+body+"'")

def main():
    #output_fims_msg("set", "/sys/foo", "/me", "{some body}")
    #output_fims_msg("set", "/sys/foo", "/me", "" )
    #output_fims_msg("set", "/sys/foo", "", "{some body}")
    #sys.exit()
    prefix = ""
    if len(sys.argv) < 2:
        usage()
    ...
    parse_fims_dump(sys.argv[1], prefix)
